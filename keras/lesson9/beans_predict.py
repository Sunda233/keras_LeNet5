...
dense = Dense(units=8, activation='relu', input_dim=2)  # 神经元数量，激活函数类型，输入特征的维度
model.add(dense)  # 将dense堆叠到数列上
model.add(Dense(units=8, activation='relu'))  # 堆叠一个输出层的单神经元dense,汇合隐藏层的两个神经元，keras会自动求导
...
plot_utils.show_scatter_surface(X, Y, model)  # 同时绘制散点图和预测曲线
# print(model.get_weights())  # 打印模型的各个参数
# 底层逻辑用线性函数组合用激活函数输出
